...
\begin{document}
	...
	\section{محاسبه وزن الگوریتم}
		...
		\subsection{ضرایب} \label{weitghs}
			...
			{\normalsize 
				...
				مقدار \lr{heuristic\_weight} نیز ضریب تاثیر \lr{heuristic} برای قسمت \ref{calculate_func} است.
			}
		...
		\subsection{محاسبه} \label{calculate_func}
			...
			{\normalsize 
				با استفاده از ضرایب دریافتی از \ref{weitghs} وزن دایچسترا مسیرها را محاسبه و برمی‌گردانیم.این تابع در کلاس \lr{Eِdge} استفاده می‌شود. سپس باید مقدار \lr{heuristic} را بر اساس فاصله جغرافیای مقصد مسیر تا مقصد نهایی حساب کنیم که بعد از آن با وزن \lr{heuristic\_weight} از \ref{weitghs} با وزن دایجسترا جمع و وزن نهایی مسیر برگردانده می‌شود.
				}
	...
	\section{کلاس \lr{Node}} \label{node_class}
		...
		{\normalsize 
			این کلاس برای ذخیره هر گره (فرودگاه) استفاده می‌شود. مقادیر
			 \lr{Airport}،‌ \lr{Airport-Country}، \lr{Ariport-City}، \lr{Latitude}، \lr{Longitude} و \lr{Altitude}
			  را دریافت و ذخیره می‌کند و همچنین از متد \lr{--str--} برای ایجاد خروجی استفاده می‌شود.
			  
			  مقادیر \lr{Latitude}، \lr{Longitude} و \lr{Altitude} برای محاسبه تابع \lr{heuristic} در \ref{calculate_func} استفاده می‌شود. 
			}
	...
	\section{کلاس \lr{Edge}} \label{edge_class}
		...
		تابع \lr{calculate} در این قسمت استفاده می‌شود تا وزن مسیر را حساب کند.
	...
	\section{لیست یکتا فرودگاه‌ها} \label{unique_nodes}
		...
		{\normalsize 
			در این قسمت از پروژه فرودگاه‌ها را به صورت یکتا \lr{(Unique)} ذخیره می‌کنیم.
	...
	\section{لیست کلاس فرودگاه‌ها \lr{(Node class)}} \label{node_list}
		...
		{\normalsize 
			با استفاده از قسمت \ref{unique_nodes} لیستی از کلاس‌های \lr{Node} می‌سازیم که برای پردازش خروجی مورد استفاده قرار می‌گیرد.
		}
	...
	\section{پردازش ورودی} \label{input_proccess}
		...
		{\normalsize 
			...
			مقدار \lr{DestinationPathClass} در قسمت \ref{edges_list} مورد استفاده قرار می‌گیرد.
		}
	...
	\section{لیست پروازها} \label{edges_list}
		...
		{\normalsize 
			با استفاده از \ref{unique_nodes} و \ref{node_list} لیستی از مسیر ها می‌سازیم که از مقادیر مقصد (\lr{nodes index})، مبدا (\lr{nodes index}) و کلاس قسمت \ref{edge_class} تشکیل می‌شود. این لیست ورودی تابع \ref{calculate_func} می‌باشد.
		}
